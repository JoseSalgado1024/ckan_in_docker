...
if [ "$exit_code" -eq "0" ] ; then
	...
	# FIX GROUPS BUG
	rm -f $CKAN_HOME/src/ckanext-gobar-theme/ckanext/gobar_theme/templates/package/snippets/package_basic_fields.html
	 cat $CKAN_HOME/src/ckanext-gobar-theme/ckanext/gobar_theme/routing.py | grep -v /dataset/groups/{id:.*?} > /tmp/tmp.file && mv /tmp/tmp.file $CKAN_HOME/src/ckanext-gobar-theme/ckanext/gobar_theme/routing.py
	...
	service nginx start && service nginx reload && service nginx restart
	service apache2 start && service apache2 reload && service apache2 restart


	# Conectamos los logs de ckan con la salida de "docker logs"
	tail  -f -n 300 /var/log/apache2/ckan_default.error.log	
